...
class RolloutStorage:
    ...
    def __init__(
        ...
        if self.num_modules == 1:
            buff_dim = (
                numsteps + 1, num_envs, num_recurrent_layers,
                recurrent_hidden_state_size
            )
        else:
            buff_dim = (
                numsteps + 1, num_envs, num_recurrent_layers,
                self.num_modules, recurrent_hidden_state_size
            )
    ...
    def to_fp_16(self):
        self.buffers.map_in_place(lambda v: v.to(dtype=torch.float16))
    ...
    def compute_returns(self, next_value, use_gae, gamma, tau):
        if use_gae:
            ...
            for step in reversed(range(self.current_rollout_step_idx)):
                ...
                #debug
                #if np.isnan(gae.item()) or np.isnan(self.buffers["value_preds"][step].item()):
                 #   print("a")
