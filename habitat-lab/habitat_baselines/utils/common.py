...
def generate_video(
    ...
    if "wandb" in video_option:
        assert wandb_ctx is not None
        # print(type(images[0]), np.max(images[0]))
        wandb_ctx.log(
            {"video": wandb.Video(list_to_tensor(images), fps=fps,
                                  format="gif"),
             "checkpoint_id": checkpoint_idx, "episode_id": episode_id}
        )


def list_to_tensor(l_img: List[np.ndarray]) -> np.ndarray:
    # print(np.transpose(l_img[0][np.newaxis, :], (0,3,1,2)).shape)
    return np.concatenate(
        [np.transpose(np.expand_dims(x, axis=0), (0, 3, 1, 2)) for x in l_img],
        axis=0)
    # pass #tocomplete
...
def image_resize_square(
    ...
    Args:
        img: the array object that needs to be resized (HWC) or (NHWC)
        size: the size that you want the shortest edge to be resize to
        channels_last: a boolean that channel is the last dimension
    Returns:
        The resized array as a torch tensor.
    """
    img = torch.as_tensor(img)
    ...
    img = torch.nn.functional.interpolate(
        img.float(), size=(size, size), mode="area"
    ).to(dtype=img.dtype)
    if channels_last:
        if len(img.shape) == 4:
            # NCHW -> NHWC
...
def is_fp16_autocast_supported() -> bool:
    return StrictVersion(torch.__version__.split('+')[0]) >= StrictVersion("1.7.1")
