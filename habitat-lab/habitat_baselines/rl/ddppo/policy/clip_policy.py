...
from torchvision.transforms import RandomCrop, RandomErasing, Resize
...
class PointNavClipPolicy(Policy):
    def __init__(
        self,
...
class ClipEncoder(nn.Module):
    def __init__(
        ...
        if normalize_visual_inputs:
            self.r_m_v_depth: nn.Module = RunningMeanAndVar(
                self._n_input_depth
            ) if self._n_input_depth > 0 else None
            self.r_m_v_rgb = RunningMeanAndVar(
                self._n_input_rgb
            ) if self._n_input_rgb > 0 else None
        else:
            self.r_m_v_rgb = nn.Sequential()
            self.r_m_v_depth = nn.Sequential()

        if not self.is_blind:
            out_dim = 0
            self.backbone = clip.load(backbone)[0].visual
            self.backbone.train()
            ...
            if self._n_input_rgb > 0:
                out_dim += self.backbone.output_dim
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:  # type: ignore
        ...
        if self._n_input_rgb > 0:
            ...
            if self.random_crop_transform is not None:
                rgb_observations = self.random_crop_transform(rgb_observations)
            if self.random_cutout_transform is not None:
                rgb_observations = self.random_cutout_transform(rgb_observations)
            #rgb_observations = F.avg_pool2d(rgb_observations, 2)

            rgb_observations = self.r_m_v_rgb(rgb_observations)

            x.append(self.backbone(rgb_observations.half()))
        ...
        if self._n_input_depth > 0:
            ...
            if self.random_crop_transform is not None:
                depth_observations = self.random_crop_transform(depth_observations)
            if self.random_cutout_transform is not None:
                depth_observations = self.random_cutout_transform(depth_observations)
            #depth_observations = F.avg_pool2d(depth_observations, 2)

            depth_observations = self.r_m_v_depth(depth_observations)

            depth_observations = depth_observations.repeat(1,3,1,1)

            x.append(self.backbone(depth_observations.half()))

        return torch.cat(x, dim=1)


class PointNavClipNet(Net):
    """Network which passes the input image through ViT and concatenates
    ...
    def __init__(
        ...
        if ImageGoalSensor.cls_uuid in observation_space.spaces:
            ...
            self.goal_visual_encoder = ClipEncoder(
    ...
    def forward(
        ...
        if not self.is_blind:
            ...
            visual_feats = self.visual_fc(visual_feats.float())
