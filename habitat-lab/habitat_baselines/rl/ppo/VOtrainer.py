...
from habitat_baselines.visual_odometry.dataset import VODataset
from habitat_baselines.visual_odometry.model import VOModel, VOPLModule


@baseline_registry.register_trainer(name="vo")
class VOTrainer(BaseRLTrainer):
    r"""Trainer, evaluator and extractor for VO.
    """
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config, aux_tasks) -> None:
        ...
        #assert
        if self.config.RL.POLICY.name in POLICY_CLASSES:
    ...
    def train_vo(self):
        c_opt = self.config["opt"]

        total_data = VODataset.from_config_path("./habitat_baselines/visual_odometry/config_base.yaml")
        import math
        #math.floor(0.7 * len(total_data)) + math.floor(
            #0.30001 * len(total_data))
        train_data, val_data = random_split(total_data, [math.floor(0.7*len(total_data)), math.floor(0.30001*len(total_data))], generator=torch.Generator().manual_seed(42))
        #val_data = VODataset.from_config_path("./habitat_baselines/visual_odometry/config_base.yaml") #to generate data or split

        t_load = DataLoader(train_data, batch_size=c_opt["b_size"])
        v_load = DataLoader(val_data, batch_size=c_opt["b_size"])
        model = VOModel.from_config_path("./habitat_baselines/visual_odometry/config_base.yaml").to("cuda")

        optim = Adam(model.parameters(), lr=float(c_opt["lr"]))

        same = 0
        v_m_loss_min = 1000000.
        for epoch in tqdm.tqdm(range(c_opt["max_iter"])):
            model.train()
            losses = []
            for batch in t_load:
                vis, pa, delta = batch
                delta_p = model(vis.to("cuda"), pa.to("cuda"))
                delta = torch.cat([delta["translation"], delta["rotation"].unsqueeze(-1)],dim=1).to("cuda")
                loss = torch.nn.functional.mse_loss(delta_p, delta.float())
                loss.backward()
                optim.step()
                optim.zero_grad()
            ...
            with torch.no_grad():
                ...
                for b in v_load:
                    vis, pa, delta = b
                    delta_p = model(vis.to("cuda"), pa.to("cuda"))
                    delta = torch.cat([delta["translation"],
                                       delta["rotation"].unsqueeze(-1)], dim=1).to("cuda")
                    loss = torch.nn.functional.mse_loss(delta_p, delta.float())
                    v_losses.append(loss.item())

                v_m_loss = np.mean(v_losses)
                if v_m_loss_min <= v_m_loss:
                    same += 1
                else:
                    v_m_loss_min = v_m_loss
                    same = 0
                self.wandb_ctx.log({
                    "epoch": epoch,
                    "val_loss": v_m_loss
            ...
            if same == c_opt["patience"]:
                print("stopped at epoch {}".format(epoch))
                break

        torch.save(model.state_dict(), "vomodel.pth")
    ...
    def _eval_checkpoint(
        ...
        while (
            ...
            error = batch["pointgoal_with_gps_compass"]-batch["odometric_gpscompass"]
            #for i in range(self.config.NUM_ENVIRONMENTS):
             #   error_stats[i].append()
              #  self.wandb_ctx.log({
               #     "error_dist": error[i, 0].item(),
                #    "error_angle": error[i, 1].item(),
                 #   "error_step": len(rgb_frames[i])
                #})
            batch = apply_obs_transforms_batch(batch, self.obs_transforms)
            ...
            for i in range(n_envs):
                ...
                if not not_done_masks[i].item():
                    ...
                    rgb_frames[i] = []
                    error_seqs.append(error_stats[i])
                    error_stats[i] = []
                ...
                # episode continues
                    ...
                    error_stats[i].append({
                        "error_dist": np.absolute(error[i, 0].item()),
                        "error_angle": error[i, 1].item(),
                    })
                else:
        ...
        for m in ["error_dist", "error_angle"]:
            dist_seq = [[e[m] for e in s] for s in error_seqs]
            for i in range(500):
                l_e_dat = {
                    m+str(j): seq[i]
                    for j, seq in enumerate(dist_seq) if i < len(seq)
                }
                l_e_dat["e-step"] = i
                self.wandb_ctx.log(l_e_dat)

            from matplotlib import pyplot as plt

            x = list(range(500))
            y = [np.mean([seq[i] for seq in dist_seq if i < len(seq)]) for i in x]
            error = [ci([seq[i] for seq in dist_seq if i < len(seq)]) for i in x]

            plt.plot(x, y, 'k-')
            plt.fill_between(x, np.asarray(y) - error, np.asarray(y) + error)
            plt.title(m)
            plt.show()
    ...
    def collect_dataset(self):
        """executes policy and collects pairs of image for VO training"""
        self.device = (
        ...
        data_path = os.path.join(self.config.DATA_DIR, self.config.SPLIT)
        os.makedirs(data_path, exist_ok=True)
        only_checkpoint = self.config["CHECKPOINT_FOLDER"] + "/" + os.listdir(self.config["CHECKPOINT_FOLDER"])[0]

        ckpt_dict = self.load_checkpoint(only_checkpoint, map_location="cpu")
        ...
        config.defrost()
        config.TASK_CONFIG.DATASET.SPLIT = config.SPLIT #config.EVAL.SPLIT
        config.freeze()
        ...
        while (
            ...
            for i in range(n_envs):
                ...
                if not not_done_masks[i].item():
                    ...
                    episode_stats['num_steps'] = len(frame_data[i])
                    ...
                    #generate pairs data
                    for j, d in enumerate(frame_data[i]):
                        #if "rgb" in d:
                         #   torch.save(d["rgb"], "vodata/rgb_{}.pt".format(n_frames))
                        #if "depth" in d:
                         #   torch.save(d["depth"], "vodata/depth_{}.pt".format(n_frames))
                        torch.save(d["rgbd"],
                                   "{}/rgbd_{}.pt".format(data_path,n_frames))
                        torch.save(d["gs"], "{}/gs_{}.pt".format(data_path,n_frames))
                        torch.save(d["pa"], "{}/pa_{}.pt".format(data_path,n_frames))

                        if j != 0:
                            entries.append((n_frames-1, n_frames))
                        n_frames += 1
                ...
                # episode continues
                    ...
                    vis = []
                    if "rgb" in batch:
                        vis.append(torch.clone(batch["rgb"][i]))
                    if "depth" in batch:
                        vis.append(torch.clone(batch["depth"][i]))
                    data["rgbd"] = torch.cat(vis, dim=2)
                    data["gs"] = torch.cat([batch["agps"][i], batch["arot"][i]], dim=0)
                    data["pa"] = torch.clone(prev_actions[i])
                    frame_data[i].append(data)
        ...
        with open("{}/entries.json".format(data_path), 'w') as f:
            json.dump(entries, f)
