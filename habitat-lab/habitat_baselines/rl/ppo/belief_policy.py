...
from habitat_baselines.rl.models.rnn_state_encoder import RNNStateEncoder, GRUStateEncoder
import habitat_baselines.rl.ddppo.policy.resnet_policy as resnet_policy
import habitat_baselines.rl.ddppo.policy.resnet as resnet
...
class SingleBelief(Net):
    ...
    def _initialize_goal_encoder(self, observation_space):
        ...
        self.goal_embedder = nn.Embedding(
            goal_space.high[0] - goal_space.low[0] + 1, self._n_input_goal)

    def _initialize_state_encoder(self):
        self.state_encoder = GRUStateEncoder(self._embedding_size, self._hidden_size)
        #RNNStateEncoder(self._embedding_size,
         #                                    self._hidden_size)
    ...
    def forward(self, visual_embedding, observations, rnn_hidden_states,
                prev_actions, masks):
...
class BeliefPolicy(Policy):
    ...
    def __init__(
        ...
        **kwargs,  # Note, we forward kwargs to the net
        ...
        assert issubclass(net,
                          SingleBelief), "Belief policy must use belief net"
        super().__init__(net(
            observation_space=observation_space,
            hidden_size=hidden_size,
            config=config,  # Forward
            **kwargs,
        ), action_space.n, action_distribution_type=config.action_distribution_type)
        self.aux_tasks = aux_tasks

        resnet_baseplanes = 32
        backbone = "resnet18"

        visual_resnet = resnet_policy.ResNetEncoder(
            observation_space,
    ...
    def act(
        ...
        **kwargs
        ...
        features, rnn_hidden_states = self.net(
            visual_embedding, observations, rnn_hidden_states, prev_actions,
            masks
        )
    ...
    def shape_aux_inputs(self, sample, final_rnn_state):
        ...
        n = final_rnn_state.size(0)  # changed from 1 to 0
        masks = sample["masks"].view(-1, n)
        ...
        t = masks.size(0)
        actions = sample["actions"].view(t, n, -1)
        vision_embedding = self.visual_encoder(observations).view(t, n, -1)

        return observations, actions, vision_embedding, n, t, env_zeros

    def evaluate_aux_losses(self, sample, final_rnn_state, rnn_features,
                            *args):
        if len(self.aux_tasks) == 0:
            pass
        observations, actions, vision_embedding, n, t, env_zeros = self.shape_aux_inputs(
            sample, final_rnn_state)
        belief_features = rnn_features.view(t, n, -1)
        final_belief_state = final_rnn_state[-1]  # only use final layer
        return [task.get_loss(observations, actions, vision_embedding,
                              final_belief_state, belief_features, n, t,
                              env_zeros) for task in self.aux_tasks]
  ...
  #      self,
   #     observation_space,
    ...
    #def visual_encoder(self, observations):
        ...
        #representation = visualpriors.representation_transform(o_t,
         #                                                      self.medium)
...
class MultipleBeliefNet(SingleBelief):
    ...
    def _initialize_state_encoder(self):
        self.state_encoders = nn.ModuleList([
            GRUStateEncoder(self._embedding_size, self._hidden_size) for _ in
    ...
    def forward(self, visual_embedding, observations, rnn_hidden_states,
        ...
        outputs = [encoder(x, rnn_hidden_states[:, :, i], masks) for i, encoder
                   in enumerate(self.state_encoders)]
        embeddings, rnn_hidden_states = zip(*outputs)  # (txn)xh, (layers)xnxh
        rnn_hidden_states = torch.stack(rnn_hidden_states,
                                        dim=-2)  # (layers) x n x k x h
...
class MultipleBeliefPolicy(BeliefPolicy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert len(
            aux_tasks) != 1, "Multiple beliefs requires more than one auxiliary task"
        assert issubclass(net,
                          MultipleBeliefNet), "Multiple belief policy requires compatible multiple belief net"
        super().__init__(
            observation_space,
            action_space,
            hidden_size,
    ...
    def evaluate_aux_losses(self, sample, final_rnn_state, rnn_features,
        ...
        return [task.get_loss(observations, actions, vision_embedding,
                              ...
                              individual_rnn_features[:, i].contiguous().view(
                                  t, n, -1), n, t, env_zeros) \
...
class AttentiveBelief(MultipleBeliefNet):
    ...
    def _fuse_beliefs(self, beliefs, x, *args):
        ...
        scores = torch.bmm(beliefs, key.transpose(1, 2)) / math.sqrt(
            self.num_tasks)  # scaled dot product
...
class FixedAttentionBelief(MultipleBeliefNet):
    ...
    def _fuse_beliefs(self, beliefs, x, *args):
        txn = x.size()[0]
        weights = torch.zeros(txn, self.num_tasks, dtype=torch.float32,
...
class AverageBelief(MultipleBeliefNet):
    def _fuse_beliefs(self, beliefs, x, *args):
        txn = x.size(0)
        weights = torch.ones(txn, self.num_tasks, dtype=torch.float32,
                             device=beliefs.device)
...
class AttentiveBeliefPolicy(MultipleBeliefPolicy):
    ...
    def from_config(cls, config, observation_space, action_space):
        return cls(observation_space, action_space, config.RL.PPO.hidden_size, AttentiveBelief)
