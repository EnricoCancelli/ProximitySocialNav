...
@registry.register_sensor(name="AGPSSensor")
class AbsoluteGPSSensor(Sensor):
...
@registry.register_sensor(name="ARotSensor")
class AbsoluteRotationSensor(Sensor):
    r"""The agents current location in the coordinate frame defined by the episode,
    i.e. the axis it faces along and the origin is defined by its state at t=0
    ...
    def get_observation(
        ...
        return quaternion.as_float_array(agent_rot)
...
class GfxReplay(Measure):
    r"""gfxreplay
    """

    cls_uuid: str = "gfx_replay"
    ...
    def __init__(
        ...
        self._sim = sim
        self._config = config
        self._gfx_keyframes_string = None
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        self._glx_keyframes_string = None
    ...
    def update_metric(
        ...
        if (
            ...
            #if not self._gfx_keyframes_string:
                #self._gfx_keyframes_string = (
                    #self._sim.gfx_replay_manager.write_saved_keyframes_to_string()
                #)
            if not task._check_episode_is_active():
                print("saving episode gfx")
                #filepath = (
                            #self._config.GFX_REPLAY_DIR
                            #+ "/episode{}.replay.json".format(
                                #episode.episode_id
                            #)
                        #)

                #self._sim.gfx_replay_manager.write_saved_keyframes_to_file(filepath)
                #self._metric = 1.
        else:
            raise Exception("replay not active. set flag or remove measure")
...
class Success(Measure):
    ...
    def update_metric(
        ...
        if (
            ...
            #if "human_collision" in task.measurements.measures and task.measurements.measures["human_collision"].get_metric():
             #   self._metric = 0.0
            #else:
                self._metric = 1.0
        else:
...
def euler_to_quaternion(r):
    ...
    qw = np.cos(roll / 2) * np.cos(pitch / 2) * np.cos(
        ...
        yaw / 2)
    return magnum.Quaternion((qx, qz, qy), qw).normalized()
...
class VelocityAction(SimulatorTaskAction):
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        #print("\n\nmustcallstop: {}\n\n".format(self.must_call_stop))
        self.time_step = config.TIME_STEP
        self.noisy_actuation = config.NOISY_ACTUATION
        self.noise_dist = (_TruncatedMultivariateGaussian([0.014, 0.009], [0.006, 0.005]), _TruncatedMultivariateGaussian([0.023], [0.012]))
    ...
    def step(
        ...
        **kwargs: Any,
        ...
        #insert actuation noise
        if self.noisy_actuation:
            pp_trans = (agent_state.position - goal_rigid_state.translation)
            theta = math.atan2(
                goal_rigid_state.translation[2] - agent_state.position[2],
                goal_rigid_state.translation[0] - agent_state.position[0]
            )
            #pp_rot = heading_to_quaternion(-theta)
            #pp_t_rot = heading_to_quaternion(-theta-np.pi/2)

            trunc = [(-0.95 * np.linalg.norm(pp_trans), None), None]

            local_translation_noise = self.noise_dist[0].sample(trunc)

            #local_translation_noise[0] *= np.sign(pp_trans + 1e-8)[0]
            #local_translation_noise[1] *= np.sign(pp_trans + 1e-8)[2]

            trunc_r = [(-0.95 * np.abs(ang_vel*self.time_step), None)]

            local_rot_noise = self.noise_dist[1].sample(trunc_r)

            local_rot_noise = local_rot_noise * np.sign(ang_vel + 1e-8)

            #plt.scatter(agent_state.position[0], agent_state.position[2], c="r")
            #plt.scatter(goal_rigid_state.translation[0], goal_rigid_state.translation[2], c="g")

            #plt.arrow(agent_state.position[0], agent_state.position[2], 0.5*np.cos(theta), 0.5*np.sin(theta), color="b")

            noisy_translation = magnum.Vector3(
                goal_rigid_state.translation[0] + np.cos(theta) * local_translation_noise[0] + np.cos(theta + np.pi / 2) * local_translation_noise[1],
                goal_rigid_state.translation[1],
                goal_rigid_state.translation[2]+np.sin(theta)*local_translation_noise[0]+np.sin(theta+np.pi/2)*local_translation_noise[1])
            #plt.scatter(
             #   goal_rigid_state.translation[0]+np.cos(theta)*local_translation_noise[0]+np.cos(theta+np.pi/2)*local_translation_noise[1],
              #  goal_rigid_state.translation[2]+np.sin(theta)*local_translation_noise[0]+np.sin(theta+np.pi/2)*local_translation_noise[1],
               # c="y")
            #for i in range(10):
                #local_translation_noise = self.noise_dist[
                 #   0].sample(trunc)
                #plt.scatter(
                 #   goal_rigid_state.translation[0] + np.cos(theta) *
                  #  local_translation_noise[0] + np.cos(theta+np.pi/2) *
                   # local_translation_noise[1],
                    #goal_rigid_state.translation[2] + np.sin(theta) *
                    #local_translation_noise[0] + np.sin(theta+np.pi/2) *
                    #local_translation_noise[1],
                    #c="black")


            #n_heading = -quat_to_rad(np.quaternion(goal_rigid_state.rotation.scalar, *goal_rigid_state.rotation.vector)) + np.pi / 2

            mn_quat_noise = euler_to_quaternion((local_rot_noise[0],0.0,0.0))

            npnoisy_rotation = np.quaternion(goal_rigid_state.rotation.scalar,
                          *goal_rigid_state.rotation.vector) * np.quaternion(
                mn_quat_noise.scalar, *mn_quat_noise.vector)

            noisy_rotation = magnum.Quaternion(npnoisy_rotation.vec, npnoisy_rotation.w)


            #noise_heading = -quat_to_rad(
                #np.quaternion(goal_rigid_state.rotation.scalar,
                              #*goal_rigid_state.rotation.vector)*np.quaternion(mn_quat_noise.scalar, *mn_quat_noise.vector)) + np.pi / 2

            #plt.arrow(goal_rigid_state.translation[0], goal_rigid_state.translation[2],
                      #0.5*np.cos(n_heading+np.pi), 0.5*np.sin(n_heading+np.pi), color="cyan" if lin_vel >= 0 else "red")
            #plt.arrow(goal_rigid_state.translation[0], goal_rigid_state.translation[2],
                      #0.5 * np.cos(noise_heading+np.pi), 0.5 * np.sin(noise_heading+np.pi),
                      #color="y")

            #for i in range(10):
                #local_rot_noise = self.noise_dist[
                 #   1].sample(trunc_r)
                #local_rot_noise = local_rot_noise * np.sign(ang_vel + 1e-8)
                #mn_quat_noise = euler_to_quaternion(
                 #   (local_rot_noise[0], 0.0, 0.0))
                #noise_heading = -quat_to_rad(
                 #   np.quaternion(goal_rigid_state.rotation.scalar,
                  #                *goal_rigid_state.rotation.vector) * np.quaternion(
                   #     mn_quat_noise.scalar,
                    #    *mn_quat_noise.vector)) + np.pi / 2
                #plt.arrow(goal_rigid_state.translation[0],
                 #         goal_rigid_state.translation[2],
                  #        0.5 * np.cos(noise_heading + np.pi),
                   #       0.5 * np.sin(noise_heading + np.pi),
                    #      color="green")
            #plt.axis("equal")
            #plt.show()
            #print(2)
        ...
        if self.noisy_actuation:
            final_position = step_fn(
                agent_state.position, noisy_translation
            )
            final_rotation = [
                *noisy_rotation.vector,
                noisy_rotation.scalar
            ]
        else:
            final_position = step_fn(
...
class NavigationTask(EmbodiedTask):
    ...
    def get_metrics_at_episode_end(self, episode: Episode):
